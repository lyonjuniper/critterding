...
struct CVar
{
	public:
		...
		const int		getIntValue() const { return m_int_value; }
		const int		getIntMin() const { return m_int_min; }
		const int		getIntMax() const { return m_int_max; }
		const int*		getIntValuePointer() const { return &m_int_value; }
		const std::string& 	getStringValue() const { return m_string_value; }
		const cvartype		getType() const { return m_type; }
