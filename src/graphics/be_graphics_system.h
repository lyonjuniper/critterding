...
public:
	...
	{
		std :: map < std :: string, BeGraphicsMaterial >:: iterator it=m_graphicsMaterialMap.find(name);
	...
	void reset3D(const int width, const int height);
...
private:
	...
	std::map<std::string, BeGraphicsMaterial > m_graphicsMaterialMap;
};
