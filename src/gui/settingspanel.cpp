...
// 	addWidgetText( strval, posx+col2, posy+row+9, settings->getCVarPtr(name) );
	addWidgetButton( strdec, Vector2i(posx+col3+m_margin, posy+row), Vector2i(11, 10), "-", Vector2i(3, 8), cmd1, EVENT_REPEAT, 150 );
	addWidgetButton( strinc, Vector2i(posx+col3+18+m_margin, posy+row), Vector2i(11, 10), "+", Vector2i(1, 8), cmd2, EVENT_REPEAT, 150 );
	row += rowspacer;
}
...
// 	addWidgetText( str, posx+col1, posy+row+9, name );
	addWidget(strval, Vector2i(posx+col2+m_margin, posy+row+9), BeWidgetTextCVARuintPtr(new BeWidgetTextCVARuint(settings->getCVarPtr(name))));
// 	addWidgetText( strval, posx+col2, posy+row+9, settings->getCVarPtr(name) );
	addWidgetButton( strdec, Vector2i(posx+col3+m_margin, posy+row), Vector2i(11, 10), "-", Vector2i(3, 8), BeCommand("settings_decrease", name), EVENT_REPEAT, 150 );
	addWidgetButton( strinc, Vector2i(posx+col3+18+m_margin, posy+row), Vector2i(11, 10), "+", Vector2i(1, 8), BeCommand("settings_increase", name), EVENT_REPEAT, 150 );
	row += rowspacer;
}
