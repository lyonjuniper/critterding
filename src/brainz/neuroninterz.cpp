...
void NeuronInterz::connec( float *output, const float synapticWeight ) // unsigned int dendrm_iteBranch, 
{
...
void NeuronInterz::process()
{
	...
	// add connected neurons output (synapse) to m_potential
		for ( m_syn = m_syn_beginP; m_syn != m_syn_endP; ++m_syn )
			m_potential += m_syn->weight * *m_syn->ref; // m_it->dendrm_iteBranch * 
