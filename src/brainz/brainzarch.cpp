...
// 			as.dendriteBranch = randgen->Instance()->get( 1, ArchNeurons[parentneuron].dendridicBranches );
	...
	const int BrainzArch::findSensorNeuron( const unsigned int id ) const
	{
		int count(0);
...
// 		std::cerr << "brain findSensorNeuron error for id: " << id << std::endl;
	...
	const bool BrainzArch::sensorNeuronExists( const unsigned int id ) const
	{
		const auto& end(InputIDs.end());
		for ( auto it(InputIDs.begin()); it != end; ++it )
		{
...
// 		std::cerr << "brain findSensorNeuron error for id: " << id << std::endl;
	...
	const int BrainzArch::findMotorNeuron( const unsigned int id ) const
	{
