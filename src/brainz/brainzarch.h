...
// 				unsigned int			mutate_maxDendridicBranches;
	...
	// RUN TIME
		...
		// functions
			const int	findSensorNeuron( const unsigned int id ) const;
			const bool	sensorNeuronExists( const unsigned int id ) const;
			const int	findMotorNeuron( const unsigned int id ) const;
	private:
		// utilities
			BeParser		parseH;
