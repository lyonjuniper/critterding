...
void CritterB::procInputNeurons()
{
	...
	// can procreate sensor neuron
		canProcreate	= false;
		if ( procreateTimeCount > (unsigned int)*critter_procinterval && energyLevel > (unsigned int)*critter_minenergyproc )
		{
	...
	// age neurons
	{
		const unsigned int NeuronToFire((int)(((float)totalFrames / (*critter_maxlifetime+1)) * 10) + overstep);
		const unsigned int count(10 + overstep);
		while ( overstep < count )
		{
			if ( overstep <= NeuronToFire )	brain.Inputs[overstep++].output = 1; // !!! <=
...
// 		glBindRenderbufferEXT(GL_RENDERBUFFER_EXT, depth_rb);
		...
		glLoadIdentity();*/
// division
		place( framePosX, framePosY, genotype->bodyArch->retinasize, genotype->bodyArch->retinasize );
	}

	void CritterB::place( const unsigned int pos_x, const unsigned int pos_y, const unsigned int size_x, const unsigned int size_y )
	{
		...
		btTransform t2( t * body.mouths[0]->ghostObject->getWorldTransform().inverse() );
