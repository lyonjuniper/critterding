...
{
	totalWeight += weight;
// 			std::cout << totalWeight << " =total " << std::endl;
...
// 		offset.setOrigin(startOffset);
		...
		{
			// calculate weight
			float weight = ((bp->x*bp->y*bp->z)/100) * 0.001f; // FIXME 0.001 is density of material
// 			std::cout << bp->x << " w " << bp->y << " x " << bp->z << " = " << weight << std::endl;
			totalWeight += weight;
// 			std::cout << totalWeight << " =total " << std::endl;
