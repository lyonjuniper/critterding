...
// 	BeWidgetPtr w = addWidgetButton( "bv_close", Vector2i(m_dimensions.m_x-25, 10), Vector2i(15, 15), "x", BeCommand("gui_togglepanel", "brainview"), EVENT_NOREPEAT, 0 );
	BeWidgetPtr w = addWidgetButton( "bv_close", Vector2i(10, 10), Vector2i(15, 15), "x", BeCommand("gui_togglepanel", "brainview"), EVENT_NOREPEAT, 0 );
...
// 			for ( unsigned int i=0; i < currentCritter->brain.numberOfInputs; ++i )
			{
				...
				else if ( (*b_inputs_it).id == 10001 )
				{
					...
					sensor s; s.sPointer = &(*b_inputs_it);
					s.m_position_x = brainview->getLocalPosition().m_x;
					s.m_position_y = 10.5f+(spacing+v_diam)*sverticalpos;
					sensors.push_back(s);
				}
				...
				else if ( (*b_inputs_it).id >= 30000 && (*b_inputs_it).id < 40000 )
				{
					...
					sensor s; s.sPointer = &(*b_inputs_it);
					s.m_position_x = brainview->getLocalPosition().m_x;
					s.m_position_y = 10.5f+(spacing+v_diam)*sverticalpos +((spacing+v_diam)*((*b_inputs_it).id-30000));
					sensors.push_back(s);
				}
				else if ( (*b_inputs_it).id >= 40000 && (*b_inputs_it).id < 50000 )
				{
					...
					sensor s; s.sPointer = &(*b_inputs_it);
					s.m_position_x = brainview->getLocalPosition().m_x;
					s.m_position_y = 10.5f+(spacing+v_diam)*sverticalpos +((spacing+v_diam)*((*b_inputs_it).id-40000));
					sensors.push_back(s);
				}
				else if ( (*b_inputs_it).id < 10000 )
				{
					...
					sensor s; s.sPointer = &(*b_inputs_it);
					s.m_position_x = brainview->getLocalPosition().m_x;
					s.m_position_y = 10.5f+(spacing+v_diam)*sverticalpos +((spacing+v_diam)*(constraintcount));
					sensors.push_back(s);
					constraintcount++;
				}
				else if ( (*b_inputs_it).id >= 50000 && (*b_inputs_it).id < 60000 )
				{
					...
					int vinput = (*b_inputs_it).id-50000;
					...
					sensor s; s.sPointer = &(*b_inputs_it);
// 					s.position = Vector2f(brainview->getLocalPosition().m_x+((spacing+v_diam)*col)+10, 10.5f+((spacing+v_diam)*row));
					s.m_position_x = brainview->getLocalPosition().m_x+((spacing+v_diam)*col)+10;
					s.m_position_y = 10.5f+((spacing+v_diam)*row);
					sensors.push_back(s);
				}
				else
				{
...
// 				}
			...
			for ( unsigned int i=0; i < s_size; ++i )
			{
				...
				{
					glColor4f(0.0f, s.sPointer->output, 0.0f, 1.0f);
				}
				// vision neurons
				else
				{
					if ( color_rotation == 0 )
						glColor4f(s.sPointer->output, 0.0f, 0.0f, 1.0f);
					else if ( color_rotation == 1 )
						glColor4f(0.0f, s.sPointer->output, 0.0f, 1.0f);
					else if ( color_rotation == 2 )
						glColor4f(0.0f, 0.0f, s.sPointer->output, 1.0f);
					else if ( color_rotation == 3 )
						glColor4f(s.sPointer->output, s.sPointer->output, s.sPointer->output, 1.0f);
